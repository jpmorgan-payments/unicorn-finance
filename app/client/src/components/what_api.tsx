        ...
        <Switch.Group>
          <div className="flex items-center ml-4">
            ...
            <Switch
              ...
              className={`${mockedDataEnabled ? 'bg-green-400 ' : 'bg-gray-200'
                } relative inline-flex items-center h-6 rounded-full w-11 txt-xs`}
            >
              <span className="sr-only">Show mocked data</span>
              <span
                className={`${mockedDataEnabled ? 'translate-x-6' : 'translate-x-1'
                  } inline-block w-4 h-4 transform bg-white rounded-full`}
              />
            </Switch>
          </div>
