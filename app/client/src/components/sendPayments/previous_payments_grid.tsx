...
const previousMockedTransactions: MockedTransactions =
  previousMockedTransactionsUntyped as MockedTransactions;
...
function PreviousPaymentsGrid() {
  ...
  const data = queryClient.getQueriesData({
    queryKey: ["globalPaymentStatus-" + currentEnvironment],
  });

  const previousPayments = useQueries({
    queries: paymentIdentifiers
      .filter((payment) => payment.environment === currentEnvironment)
      .map((id) => ({
        queryKey: ["globalPaymentStatus-" + currentEnvironment, id.endToEndId],
        queryFn: () =>
          sendGet(
            gatherPath(currentEnvironment, paymentConfig.apiDetails[1]).replace(
              "<endToEndId>",
              id.endToEndId || "",
            ),
          ),
  ...
  const previousPaymentsLoading = previousPayments.some(
  ...
  if (currentEnvironment === Environment.MOCKED) {
    mockedData = [
      ...mockedPreviousPaymentsSession,
      ...previousMockedTransactions.payments,
    ];
  }

  const renderCells = (
    payment: PaymentStatusResponseType,
    endToEndId: string,
  ) => (
    <tr
      onClick={() =>
        setJsonDialogData({
          state: true,
          data: JSON.stringify(payment, undefined, 2),
        })
      }
      key={`paymentKey-${endToEndId}`}
    >
      ...
      <td className="border-b border-slate-100  p-4 pl-8 ">
        {payment.paymentStatus?.createDateTime
          ? payment.paymentStatus?.createDateTime
          : ""}
      </td>
      <td className="border-b border-slate-100  p-4 pl-8 ">
        {payment.paymentStatus.exception
          ? `${payment.paymentStatus.exception[0].errorCode} ${payment.paymentStatus.exception[0].errorDescription ? `- ${payment.paymentStatus.exception[0].errorDescription}` : ""}`
          : ""}
      </td>
    </tr>
  );

  const renderMockedCells = () =>
    mockedData.map((mock) => renderCells(mock, mock.identifiers.endToEndId));
  ...
  const renderTable = () => (
    <table
      className="border-collapse table-layout text-md overflow-scroll w-full block lg:table"
      data-cy="previousPaymentsGrid"
    >
      <thead>
        <tr>
          {headers.map((header) => (
  ...
  const renderEmptyPrevious = () => (
    <p className="text-center text-xl mt-20">
      Send a payment to be able to track them here
    </p>
  );
  ...
  return (
    <div className="h-full">
      ...
      {displayingApiData ? (
        ...
        <Spinner text="Updating payment status data...." />
      ) : (currentEnvironment === Environment.MOCKED &&
          mockedData.length === 0) ||
        (currentEnvironment !== Environment.MOCKED &&
