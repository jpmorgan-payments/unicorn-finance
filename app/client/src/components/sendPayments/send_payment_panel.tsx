...
function MakePaymentForm() {
  const { displayingApiData, currentEnvironment } =
  ...
  const createPaymentMutation = useMutation({
    mutationFn: (data: GlobalPaymentRequest) =>
      sendPostGlobalPaymentInit(
        gatherPath(currentEnvironment, paymentConfig.apiDetails[0]),
        JSON.stringify(data),
      ),
  });
  ...
  return (
    <div className=" w-full flex flex-col h-full pb-20">
      ...
      {!displayingApiData && createPaymentMutation.isPending && (
        <div className="text-center pt-24">
          <Spinner text="Loading API Response..." />
        </div>
      )}
      {((!displayingApiData && createPaymentMutation.isSuccess) ||
        (currentEnvironment === Environment.MOCKED && apiResponse)) &&
        !apiError &&
        !apiResponse?.errors && (
          <>
